...
namespace SDRSharp.BladeRF
{
    public sealed class BladeRFDevice : IDisposable
    {
        ...
        private const uint MinFrequency = 60000000;
        ...
        private IntPtr _dev;
        private unsafe bladerf_range* _available_sample_rates;
        ...
        public int VGA2Gain
        {
            ...
            set
            {
                _vga2Gain = value;
                if (_dev != IntPtr.Zero)
                    NativeMethods.bladerf_set_gain(_dev, bladerf_module.BLADERF_MODULE_RX, _vga2Gain);
            }
        }
        ...
        public unsafe bladerf_range* Available_sample_rates { get => _available_sample_rates; set => _available_sample_rates = value; }
        ...
        public unsafe BladeRFDevice(string serial = "")
        {
            _isFpgaLoaded = false;
            string devspec = "";
            if (serial != "")
            ...
            bladerf_range* available_sample_rates;
            NativeMethods.bladerf_get_sample_rate_range(_dev, (int)bladerf_channel.BLADERF_RX_X1, &available_sample_rates);
            _available_sample_rates = available_sample_rates;
            _boardName = Marshal.PtrToStringAnsi(NativeMethods.bladerf_get_board_name(_dev));
        ...
        private unsafe void ReceiveSamples_sync()
        {
            ...
            while (_isStreaming)
            {
                ...
                while (status == 0 && cur_len == new_len && IsStreaming)
                {
                    try
                    {
        ...
        public unsafe void Start()
        {
            ...
            NativeMethods.bladerf_set_gain_mode(_dev, bladerf_module.BLADERF_MODULE_RX, bladerf_gain_mode.BLADERF_GAIN_MGC);

            if ((error = NativeMethods.bladerf_set_gain(_dev, bladerf_module.BLADERF_MODULE_RX, _vga2Gain)) != 0)
        ...
        {
            return _available_sample_rates;
        }
